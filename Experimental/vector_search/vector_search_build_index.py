...
TENSOR_DIRECTORY = "embeddings/chunked_6seconds/chunked_6s"
...
def main():
  ...
  # Load embeddings (.pt)
  # -------------------------------------
  tensors = load_tensor(TENSOR_DIRECTORY, num_workers=os.cpu_count() or 4, use_cache=False)
  if not tensors:
  ...
  for pooling_policy in POOLING_POLICY:
    ...
    index_path = f"{VECTOR_INDEX_DIR}/annoy_index_chunked_6seconds_{pooling_policy}.ann"
