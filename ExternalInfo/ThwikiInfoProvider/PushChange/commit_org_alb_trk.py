...
def load_tracks(album_map: Dict):
    ...
    for track in OriginalTrack.select(OriginalTrack, OriginalTrack.source).where(
        OriginalTrack.source.not_in(IGNORE)
    ):
...
if __name__ == "__main__":
    ...
    album_add_endpoint = (
        input("Album CREATE endpoint: ") or "http://localhost:5217/api/source/album"
    )
    track_add_endpoint = (
        input("Track CREATE endpoint: ")
        or "http://localhost:5217/api/source/album/{albumId}/track"
    )
    commit(
