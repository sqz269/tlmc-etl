...
def map_line(line: List[str]) -> dict:
    b = {
        "id": line[2],
        "type": line[1],
        "fullName": {
...
def load_original_album_map() -> Dict:
    path = (
        ...
        or r"D:\PROG\TlmcTagger\InfoProviderV3Pipeline\InfoProviders\ThcInfoProvider\ExternalData\TouhouAlbumV2.csv"
    )
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for line in csv_reader:
            original_song_map.update(map_line(line))
...
def load_tracks(album_map: Dict):
    loaded = {}
    for track in OriginalTrack.select(OriginalTrack, OriginalTrack.source).where(
        OriginalTrack.source.not_in(non_offical_works)
    ):
...
def commit(album_map, track_map, track_add_ep, album_add_ep):
    for entry in album_map.values():
        ...
        resp = requests.post(
            album_add_ep, json=entry, headers={"Content-Type": "application/json"}
        )
        ...
        for track in track_map[album_id]:
            ...
            resp = requests.post(
                track_add_ep.format(albumId=album_id),
                json=track,
...
if __name__ == "__main__":
    ...
    album_add_endpoint = (
        input("Album CREATE endpoint: ") or "http://localhost:5294/api/source/album"
    )
    track_add_endpoint = (
        input("Track CREATE endpoint: ")
        or "http://localhost:5294/api/source/album/{albumId}/track"
    )
    commit(
        original_album_map, original_track_map, track_add_endpoint, album_add_endpoint
    )
