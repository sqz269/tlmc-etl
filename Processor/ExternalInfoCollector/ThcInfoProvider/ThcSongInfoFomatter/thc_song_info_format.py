...
def load_original_song_map() -> Dict:
    path = input("Enter path to CSV with original track abbrivations (Generated by get-original-alb-trk.py): ") or r"D:\PROG\TlmcTagger\InfoProviderV3Pipeline\OriginalAlbums_Blank.csv"
...
def precheck():
    alb_no_title = Album.select().where((Album.title_jp == None) & (Album.process_status != ProcessStatus.FAILED)).count()
...
def configure_track(album: AlbumFormatted, track: Track, abbriv_map: Dict) -> TrackFormatted:
...
def import_data():
    ...
    for album in albums:
        ...
        imported_album.append(import_album)
        for track in tracks:
            print(f"Imported {imp_alb_count} albums and {imp_trk_count} tracks [{imp_mismatch_count} mismatches]", end="\r")
            imp_trk_count += 1
